...
def combined_gradient_matching(model, origin_grad, iteration, switch_iteration=100, use_tv=True):
    ...
    dummy_data = torch.randn(origin_grad[0].size(), requires_grad=True)  # Random initialization
    dummy_label = torch.randn((1, origin_grad[-1].size(1)), requires_grad=True)  # Random labels
